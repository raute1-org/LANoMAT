...
- **Steuerung = MAs WebSocket-API** (teilweise als JSON/REST gespiegelt; auto-generierte Doku unter `http://<MA>:8095/api-docs`; Referenz: offizieller Python-Client + JS/TS-Frontend). Die Contract-Verben `search`/`enqueue`/`reorder`/`skip`/`nowPlaying` bilden direkt auf MA-Queue-/Player-Kommandos ab; Now-Playing-/Queue-Zustand kommt als **WS-Push** → Infoscreen-Now-Playing-Szene + `event.{id}`-Reverb. **Im Detailplan verifizieren:** exakte WS-Kommandos gegen `:8095/api-docs`; und da LANoMAT PHP ist und die API WS-first, klären ob ein **schmaler WS-Sidecar** (analog zum Mumble-Ice-REST-Sidecar) oder ein PHP-WS-Client (Ratchet/Pawl) der sauberere Transport ist.
- **Erster M11-Task = MA-Anbindungs-Spike** (ersetzt den früheren go-librespot-Spike): einen MA-Server + einen Player (z. B. Snapcast) hochziehen, per WS `search`/`enqueue`/`reorder`/`nowPlaying` durchspielen und Now-Playing-Events empfangen — bestätigt billig, dass sich die vote-getriebene Reihenfolge in MAs Queue **spiegeln** lässt und dass der PHP↔MA-Transport (Sidecar vs. WS-Client) trägt.
